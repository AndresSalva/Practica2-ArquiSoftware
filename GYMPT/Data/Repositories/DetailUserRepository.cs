    ...
    public class DetailUserRepository : IRepository<DetailsUser>
    {
        ...
        public async Task<DetailsUser> CreateAsync(DetailsUser entity)
        {
            try
            {
                ...
                var userExists = await conn.ExecuteScalarAsync<bool>(
                    @"SELECT COUNT(1) FROM ""Users"" WHERE id = @UserId AND ""isActive"" = true",
                    new { UserId = entity.IdUser });

                if (!userExists)
                {
                    throw new ArgumentException($"El usuario con ID {entity.IdUser} no existe o no está activo.");
                }
                ...
                if (!membershipExists)
                {
                    throw new ArgumentException($"La membresía con ID {entity.IdMembership} no existe o no está activa.");
                }
        ...
        public async Task<bool> DeleteByIdAsync(long id)
        {
            try
            {
                ...
                await RemoteLoggerSingleton.Instance.LogInfo($"Eliminación completada. Filas afectadas: {affected}");
                return affected > 0;
            }
            catch (Exception ex)
            {
