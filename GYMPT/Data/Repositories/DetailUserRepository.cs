...
namespace GYMPT.Data.Repositories
{
    public class DetailUserRepository : IRepository<DetailsUser>
    {
        ...
        {
            await RemoteLoggerSingleton.Instance.LogInfo($"Creating datail user: {entity.IdUser}");
            ...
            if (!userExists) throw new ArgumentException($"User with id {entity.IdUser} doesn't exist or ísn't available.");
            ...
            if (!membershipExists) throw new ArgumentException($"The membership with id {entity.IdMembership} doesn't exist or isn't available.");
        ...
        public async Task<DetailsUser> UpdateAsync(DetailsUser entity)
        {
            ...
            if (affectedRows == 0) throw new KeyNotFoundException("Details user wasn't found for actualization.");
            return entity;
        }
    }
}